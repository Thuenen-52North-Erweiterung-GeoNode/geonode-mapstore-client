...
} from "@mapstore/framework/utils/TOCUtils";
...
function SelectGroup({
  ...
  const getLabelName = (label, groups) =>
    _getLabelName(getTitle(label), groups);
  ...
  return (
    <SelectCreatable
      ...
      value={{
        label: getLabelName(nodeGroupLabel, groups),
...
function GeneralSettings(
  ...
  function handleUpdateTranslation(key, event) {
    const title =
      key === "default" && isString(node.title)
        ? event.target.value
        : {
            ...(isObject(node.title)
              ? node.title
  ...
  const tooltipItems = [
    {
      value: "title",
      label: getMessageById(context.messages, "layerProperties.tooltip.title"),
    },
    {
      value: "description",
      label: getMessageById(
        context.messages,
        "layerProperties.tooltip.description"
      ),
    },
    {
      value: "both",
      label: getMessageById(context.messages, "layerProperties.tooltip.both"),
    },
    {
      value: "none",
      label: getMessageById(context.messages, "layerProperties.tooltip.none"),
    },
  ];
  const tooltipPlacementItems = [
    {
      value: "top",
      label: getMessageById(context.messages, "layerProperties.tooltip.top"),
    },
    {
      value: "right",
      label: getMessageById(context.messages, "layerProperties.tooltip.right"),
    },
    {
      value: "bottom",
      label: getMessageById(context.messages, "layerProperties.tooltip.bottom"),
    },
  ];
  ...
  const {
    ...
    //add "exclusiveLayer: layer.exclusiveLayer" to
    //submodule MapStore2/web/client/utils/LayerUtils.js Line 576ff
    //https://github.com/geosolutions-it/MapStore2/blob/9f545d8aaf11135d3b18a1afa036cc081081030c/web/client/utils/LayersUtils.js#L576
    exclusiveLayer = false,
  } = node || {};

  const currentTitle = isString(title)
    ? title
    : _getTitle(title, currentLocale) ?? title?.default ?? "";
  ...
  return (
    <>
      ...
      <FormGroup>
        <ControlLabel>
          <Message msgId="layerProperties.description" />
        </ControlLabel>
        <FormControl
      ...
      {nodeType === "layer" ? (
        <>
          <FormGroup>
            <ControlLabel>
              <Message msgId="layerProperties.group" />
            </ControlLabel>
            <SelectGroup
              node={node}
              onChange={onChange}
      ...
      {showTooltipOptions && (
        <>
          <FormGroup>
            <ControlLabel>
              <Message msgId="gnviewer.nodeTooltipContent" />
            </ControlLabel>
            <Select
              clearable={false}
              ...
              onChange={({ value }) =>
                onChange({ tooltipOptions: value || "title" })
              }
            />
          </FormGroup>
          <FormGroup>
            <ControlLabel>
              <Message msgId="gnviewer.nodeTooltipPlacement" />
            </ControlLabel>
            <Select
              clearable={false}
              key="tooltips-placement-dropdown"
              options={tooltipPlacementItems}
              value={tooltipPlacementItems.find(
                ({ value }) => value === tooltipPlacement
              )}
              onChange={({ value }) =>
                onChange({ tooltipPlacement: value || "top" })
              }
            />
          </FormGroup>
        </>
