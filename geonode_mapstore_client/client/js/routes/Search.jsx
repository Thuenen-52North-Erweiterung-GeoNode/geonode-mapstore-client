...
const suggestionsRequestTypes = {
  categories: {
    ...
    loadOptions: (params) =>
      getCategories(params, "filter{category.identifier.in}"),
  },
  keywords: {
  ...
  useEffect(() => {
    ...
    queryKeys.forEach((queryKey) => {
      ...
      if (suggestionRequest) {
        const filtersToUpdate = castArray(state.current.query[queryKey]).filter(
          (value) => !getFilterById(queryKey, value)
        );
        if (filtersToUpdate?.length > 0) {
          const request = suggestionRequest.loadOptions.bind(null, {
            includes: filtersToUpdate,
            pageSize: filtersToUpdate.length,
          });
...
const ConnectedSearch = connect(
  ...
  ),
  {
    ...
    onEnableFiltersPanel: setControlProperty.bind(
      null,
      "gnFiltersPanel",
      "enabled"
    ),
